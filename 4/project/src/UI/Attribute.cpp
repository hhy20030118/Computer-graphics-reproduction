...
void Attribute::ComponentVisitor::ImplVisit(Ptr<CmptSimulate> simulate) {
	...
	grid->AddEditVal({ "ks" }, simulate->Getks(),0.1f, [=](const float& val) {
		simulate->Setks(val);
		});
	grid->AddEditVal({ "mess" }, simulate->Getmess(), 0.1f, [=](const float& val) {
		simulate->Setmess(val);
		});
	grid->AddEditVal({ "kd" }, simulate->Getkd(), 0.01f, [=](const float& val) {
		simulate->Setkd(val);
		});
	...
	grid->AddButton("set points fix", [simulate]() {
		simulate->SetpointFix();
		});
	grid->AddButton("turn to half_implicit", [simulate]() {
		simulate->SetType(1);
		});
	grid->AddButton("switch mode", [simulate]() {
		simulate->SetMode();
		});
	grid->AddButton("turn to implicit", [simulate]() {
		simulate->SetType(2);
		});
	grid->AddButton("turn to fast_implicit", [simulate]() {
		simulate->SetType(3);
		});
	grid->AddButton("turn to PBD", [simulate]() {
		simulate->SetType(4);
		});
}
...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddText(" normal");
	grid->AddButton("change type to origin", [mesh, pOGLW = attr->pOGLW]() {
		mesh->boundary_type = 0;
	});
	grid->AddButton("change type to square", [mesh, pOGLW = attr->pOGLW]() {
		mesh->boundary_type = 1;
	});
	grid->AddButton("change type to circle", [mesh, pOGLW = attr->pOGLW]() {
		mesh->boundary_type = 2;
	});
	grid->AddButton("change method to origin", [mesh, pOGLW = attr->pOGLW]() {
		mesh->method = 1;
	});
	grid->AddButton("change method to cot", [mesh, pOGLW = attr->pOGLW]() {
		mesh->method = 2;
	});
	grid->AddButton("MinSurf", [mesh, pOGLW = attr->pOGLW]() {
		auto minSurf = MinSurf_normal::New(mesh);
		minSurf->Run();
		pOGLW->DirtyVAO(mesh);
	});
