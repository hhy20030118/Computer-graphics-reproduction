...
int main()
{
    ...
    auto plane = SimpleLoader::LoadObj("../data/models/square_sparse_horizon.obj");
    // world space positions of our cubes
    pointf3 instancePositions[] = {
        pointf3(0.0f,  0.0f,  0.0f),
        pointf3(0.0f,  -1.0f, 0.0f),
    ...
    gl::Texture2D plane_albedo = loadTexture("../data/textures/checkerboard.png");
    // shadow buffer
    // -------------------------
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        transformf light_projection = transformf::perspective(to_radian(100.f), 1.0f, 1.0f, 100.f);
        shadow_program.SetMatf4("projection", light_projection);

        transformf light_view = transformf::look_at(Ubpa::pointf3(0, 5, 0), Ubpa::pointf3(0, 0, 0), Ubpa::vecf3(0.0, 0.0, 1.0));
        ...
        transformf modelp(instancePositions[1], quatf{ vecf3(1.0f, 0.3f, 0.5f), to_radian(0.0) });
        shadow_program.SetMatf4("model", model);
        spot->va->Draw(&shadow_program);
        shadow_program.SetMatf4("model", modelp);
