...
void SimpleLoader::OGLResources::setpos(std::vector<pointf3> pos) {
	this->positions = move(pos);
}
SimpleLoader::OGLResources* SimpleLoader::LoadObj(const std::string& path, bool noise) {
	...
	vector<unsigned int> edges_pre;
	for (register int a = 0; a < 10000; ++a) {
		edges.push_back(edges_pre);
	}
	for (auto line : lines) {
		if (line.empty() || line == "\n")
			continue;
		...
		if (id == "v") {
			...
			bool already = 0;
			for (auto x : edges[tri[0]-1]) {
				if (x == tri[1]-1)already = 1;
			}
			if (!already)edges[tri[0]-1].push_back(tri[1]-1);
			already = 0;
			for (auto x : edges[tri[1] - 1]) {
				if (x == tri[2] - 1)already = 1;
			}
			if (!already)edges[tri[1] - 1].push_back(tri[2] - 1);
			already = 0;
			for (auto x : edges[tri[2]- 1] ) {
				if (x == tri[0] - 1)already = 1;
			}
			if (!already)edges[tri[2] - 1].push_back(tri[0] - 1);
		}
		else if (id == "vn") {
			normalf n;
			ss >> n;
